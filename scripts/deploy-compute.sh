...
if [[ -n "$REMOTE" ]]; then
    ...
    ssh "$REMOTE" "cd ~/$REMOTE_DIR && git pull && $VENV/bin/pip install -q --upgrade -r requirements.txt && $VENV/bin/pip install -q --upgrade geotessera && $VENV/bin/pip install -q --upgrade 'tessera-eval[server] @ git+https://github.com/ucam-eo/tessera-eval.git@v1.2.6' 2>&1 || true"
