"""Pure tile rendering helpers: coordinate math, PNG rendering, pyramid path resolution."""
...
def get_pyramid_path(viewport, map_id, zoom_level):
    ...
    Returns (path_str, mtime) or None if not found.
    """
    ...
    try:
        mtime = int(png_path.stat().st_mtime)
    except FileNotFoundError:
        return None

    return (str(png_path), mtime)
